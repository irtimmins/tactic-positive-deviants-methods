# Comparison of ranking methods (the basis for Table 3) and funnel plots using
# indirect standardisation. Methods compared on the sustained estimand: raw mean
# and median, weighting standardisation (age + comorbidity, and the full patient
# mix) for the mean and median, regression standardisation, the indirectly
# standardised mean, and the shrunk expected rank from the Bayesian fit.
...
# regression standardisation (g-computation): predict every hospital over the
# whole population covariate distribution and average.
...
# indirect standardisation: expected wait from a pooled model evaluated on each
# hospital's own patients, then observed - expected + grand mean.
...
methods <- c("raw_mean","raw_median","wt_ac_mean","wt_ac_med",
             "wt_full_mean","wt_full_med","reg_ac","reg_full","indirect")
ranks <- comp %>%
